...
- _"Summarize the feedback on my last submission in OM 406."_
- _"What changed in my courses since yesterday?"_
- _"Send my professor a question about Exam 2."_
...
- `reply_discussion(course_id, topic_id, message, parent_entry_id?)` — _(write)_.
- `mark_discussion_read(course_id, topic_id)` — _(write)_.
...
- `send_message(recipients[], body, subject?, context_code?, group_conversation?)` — _(write)_.
- `mark_conversation(conversation_id, state?, starred?)` — _(write)_. `state` ∈ {read, unread, archived}.
...
- `submit_assignment_text(course_id, assignment_id, body)` — _(write)_. `online_text_entry`.
- `submit_assignment_url(course_id, assignment_id, url)` — _(write)_. `online_url`.
- `submit_assignment_file(course_id, assignment_id, file_paths[], content_types?)` — _(write)_. `online_upload`. Reads files from local disk and runs Canvas's three-step upload (init → form post → submit).
- `add_submission_comment(course_id, assignment_id, text)` — _(write)_. Reply to grader feedback without creating a new submission.
